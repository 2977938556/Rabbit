<template>
    <XtxDialog v-model:visible="visibleDialog" title="查看物流" v-if="logistics">
        ...
        <div class="order-logistics-list" v-if="logistics">
            <div class="item" v-for="(item, i) in logistics.list" :key="item.id">
                <div class="line" :class="{ active: i < logistics.list.length }"></div>
                <div class="logistics">
                    <p>{{ item.text }}</p>
                    <p>2022-1-1-1</p>
                </div>
...
export default {
    ...
    setup() {
        ...
        let open = (order) => {
            ...
            // 通过`id获取物流信息
            logisticsOrder(order.id).then(({ result }) => {
